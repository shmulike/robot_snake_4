...
#define limit_max_tension 14        // String tension limit [Kg]
    ...
    { 
        ...
        else
        {
            ...
            for (int joint_i=0; joint_i<N_links; joint_i++)
            {
              ...
              /*  if(signOf(joint_error[joint_i])<0&&joint_i%2!=0 && fabs(joint_error[joint_i])>eps_angle){
                 tension_error[1][joint_i] = ((string_tension[joint_i]+4*fabs(joint_error[joint_i])) - tension_val[1][joint_i]);
                }
               else{
                ...
                for (int j=0; j<2; j++){
                    if (tension_val[j][joint_i]>limit_max_tension){
                        ...
                        motor_cmd[1][joint_i] = -MAX_PWM_tension;
